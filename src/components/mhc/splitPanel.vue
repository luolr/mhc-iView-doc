<template>
  <div class="split-panel" @mousemove="dragMove" @mouseup="dragStop"
       :class="{'dragging': dragging, 'vert': layoutType == 'vertical', 'hori': layoutType == 'horizontal'}">
    <div class="split-panel-part" :style="layoutType == 'horizontal' ? {width: panelSize1, borderColor: lineColor} : {height: panelSize1, borderColor: lineColor}">
      <slot name="1"></slot>
    </div>
    <div class="split-panel-gutter" @mousedown="dragStart" :style="layoutType == 'horizontal' ? {width: gutterSizeString, borderColor: lineColor, backgroundColor: lineColor} : {height: gutterSizeString, borderColor: lineColor, backgroundColor: lineColor}"></div>
    <div class="split-panel-part" :style="layoutType == 'horizontal' ? {width: panelSize2, borderColor: lineColor} : {height: panelSize2, borderColor: lineColor}">
      <slot name="2"></slot>
    </div>
  </div>
</template>

<script>
export default {
  props: ['layout', 'color', 'gutter', 'init', 'min', 'max'],
  data() {
    return {
      layoutType: this.layout || 'horizontal',
      lineColor: this.color || '#5cadff',
      gutterSize: this.gutter || 8,
      percent: this.init || 50,
      minPercent: this.min || 0,
      maxPercent: this.max || 100,
      dragging: false,
    };
  },
  computed: {
    gutterSizeString() {
      return `${this.gutterSize}px`;
    },
    panelSize1() {
      if (this.percent < this.minPercent) {
        this.percent = this.minPercent;
      }
      return `calc(${this.percent}% - ${this.gutterSize}px)`;
    },
    panelSize2() {
      if (this.percent > this.maxPercent) {
        this.percent = this.maxPercent;
      }
      return `${100 - this.percent}%`;
    },
  },
  methods: {
    dragStart(e) {
      e.preventDefault();
      this.dragging = true;
      this.startPos = this.layoutType === 'horizontal' ? e.pageX : e.pageY;
      this.startSplit = this.percent;
      e.stopPropagation();
    },
    dragMove(e) {
      if (this.dragging) {
        e.preventDefault();
        const dx = (this.layoutType === 'horizontal' ? e.pageX : e.pageY) - this.startPos;
        const totalSize = this.layoutType === 'horizontal' ? this.$el.offsetWidth : this.$el.offsetHeight;
        this.percent = this.startSplit + Math.round((dx / totalSize) * 100);
        e.stopPropagation();
      }
    },
    dragStop(e) {
      if (this.dragging) {
        e.preventDefault();
        this.dragging = false;
        e.stopPropagation();
      }
    },
  },
};
</script>

<style>
  *,
  *::before,
  *::after {
    box-sizing: border-box;
  }
  .split-panel {
    width: 100%;
    height: 100%;
  }
  .split-panel-part {
    display: inline-block;
    float: left;
    overflow: auto;
  }
  .split-panel.hori > .split-panel-part {
    height: 100%;
  }
  .split-panel.vert > .split-panel-part {
    width: 100%;
  }
  .split-panel-gutter {
    display: inline-block;
    background: #5cadff;
    z-index: 1;
    float: left;
    background-clip: padding-box;
  }
  .split-panel.hori > .split-panel-gutter {
    height: 100%;
    cursor: col-resize;
    border-left: 1px solid #5cadff;
    border-right: 1px solid #5cadff;
  }
  .split-panel.vert > .split-panel-gutter {
    width: 100%;
    cursor: row-resize;
    border-top: 1px solid #5cadff;
    border-bottom: 1px solid #5cadff;
  }
  .split-panel.hori > .split-panel-gutter:hover,
  .split-panel.hori > .split-panel-gutter:focus {
    border-left: 1px solid #5cadff;
    border-right: 1px solid #5cadff;
  }
  .split-panel.vert > .split-panel-gutter:hover,
  .split-panel.vert > .split-panel-gutter:focus {
    border-top: 1px solid #5cadff;
    border-bottom: 1px solid #5cadff;
  }
  .split-panel.hori.dragging {
    cursor: col-resize;
  }
  .split-panel.vert.dragging {
    cursor: row-resize;
  }
</style>